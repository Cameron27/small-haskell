...
evalPgm :: Pgm -> Ans
evalPgm (Program c owns) = evalOwn owns defaultEnv (\r -> evalCom c r (\_ -> return ExitSuccess)) store
  where
    env = defaultEnv
    store = Store HashMap.empty 0

evalOwn :: [(Int, Exp)] -> Env -> Dc -> Cc
evalOwn ((n, e1) : owns) r u = evalRVal e1 r $ ref (\l -> evalOwn owns (updateEnv (newEnvOwn n l) r) u)
evalOwn [] r u = u r
...
evalExp (Valof c1) r k s = evalCom c1 (Env r' w' k) (err $ printf "no return encountered in %s" $ show (Valof c1)) s
  where
    (Env r' w' _) = r
...
evalDec (Own i1 e1 n) r u s = u (newEnv i1 (lookupEnvOwn n r)) s
  where
    (DLoc l) = lookupEnvOwn n r
    clearEnv = let (Env _ w _) = r in updateEnv (Env HashMap.empty w emptyEc) defaultEnv
...
evalDec SkipDec r u s = u (Env HashMap.empty HashMap.empty emptyEc) s
